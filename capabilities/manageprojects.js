...
async function createProject({
  ...
  const { error: errAddProject } = await supabase
...
async function updateProject({
  ...
  if (
    ...
    (!newAliases ||
      JSON.stringify(newAliases) === JSON.stringify(projectBefore.aliases)) &&
...
async function updateProjectStatus({
  ...
  let query = supabase.from(PROJECT_TABLE_NAME).update(newValue);
